...
if [ "$BITRISE_GIT_BRANCH" == "master" ]; then git push --quiet https://$GITHUB_ACCESS_TOKEN@github.com/$ORGANIZATION/$REPOSITORY $NEW_GIT_TAG > /dev/null 2>&1; fi
if [ "$BITRISE_GIT_BRANCH" == "master" ]; then ../release_notes.sh $OLD_GIT_TAG $NEW_GIT_TAG; fi
